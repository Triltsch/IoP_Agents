...
# # AsyncUA HOT monitor client for factory machines
# 
...
async def _clear_repair_flag(machine_name: str) -> None:
    ...
    async with Client(url=SERVER_URL) as client:
        ...
        if nodes is None:
            raise ValueError(f"Unknown machine name {machine_name!r} on OPC UA server.")

        # job_node is the Boolean Mxx_RepairNeeded variable
        await nodes.job_node.write_value(ua.Variant(False, ua.VariantType.Boolean))
